...
//继承CCIPReceiver，可以调用很多关于接收ccip发来的消息的方法
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    struct RequestData {
        uint256 tokenId;  // the token id of the NFT to be locked
    ...
    constructor(address _router, address _link, address wntfAddr) CCIPReceiver(_router) {
        s_linkToken = IERC20(_link);
        wnft = WrappedMyToken(wntfAddr);
    }
    ...
    /// @notice 使用LINK支付费用。
    /// @dev 假设您的合约有足够的LINK。
    /// @param _destinationChainSelector 目标区块链的标识符（又称选择器）。
    /// @param _receiver 目标区块链上接收者的地址。
    /// @param _text 要发送的文本。
    /// @return messageId 已发送的CCIP消息的ID。
    ...
    function _ccipReceive(
        ...
        override
    {   
        //address newOwner, unt256 tokenId
